...
4. **Validation**
   - The admin runs `/spectra definition validate my-custom-meteor`.
...
5. **Preview / Test**
   - The admin uses `/spectra model preview <model-id>` to spawn the defined visual components locally and ensure they look correct.

6. **Reload Definition**
   - The admin runs `/spectra definition reload my-custom-meteor` (or `/spectra reload`).
...
7. **Manual Start**
   - The admin executes `/spectra event start my-custom-meteor`.
   - The engine selects a valid spawn location (based on the strategy) and creates a new `EventInstance`.

8. **Runtime Inspection**
   - The admin tracks the event using `/spectra event info <instance-id>` to see its current Phase, Location, and Health.
   - They can teleport to the instance via `/spectra event teleport <instance-id>`.

9. **Debug**
   - If interactions aren't working, the admin runs `/spectra debug hitboxes <instance-id>` to visualize the physical interaction boundaries.
